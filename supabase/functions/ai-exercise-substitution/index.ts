...
serve(async (req) => {
  ...
  try {
    ...
    // Validate UUID formats - treat invalid UUIDs as "not found"
    ...
    for (const exercise of sessionExercises) {
      ...
      if (!hasAllEquipment) {
        exercisesNeedingSubstitution.push({
          id: exerciseData.id,
          exercise_id: exerciseData.exercise_template_id,
          exercise_name: exerciseData.exercise_templates.name,
          prescribed_sets: exerciseData.target_sets,
          prescribed_reps: exerciseData.target_reps,
          prescribed_rpe: exerciseData.target_rpe,
...
`]
    ...
    exercisesNeedingSubstitution.forEach((exercise, idx) => {
      ...
      promptParts.push(`
${idx + 1}. ${exercise.exercise_name} (ID: ${exercise.exercise_id})
   ...
   PRE-VETTED CANDIDATES:`)
      ...
      candidates.forEach((candidate, cidx) => {
        promptParts.push(`   ${cidx + 1}. ${candidate.substitute_name} (ID: ${candidate.substitute_id})
