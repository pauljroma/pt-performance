...
class ProgramEditorViewModel: ObservableObject {
        ...
        do {
            ...
            if let exerciseId = exerciseId {
                ...
                do {
                    ...
                    selectedExercise = availableExercises.first { $0.id.uuidString == sessionExercise.exerciseTemplateId }
    ...
    func queryExercises(filters: [String: String] = [:]) async -> [Exercise] {
        ...
        do {
            ...
            let exercises = templates.compactMap { template -> Exercise? in
                guard let templateUUID = UUID(uuidString: template.id) else {
                    logger.log("⚠️ Invalid UUID for template: \(template.id)", level: .warning)
                    return nil
                }
    ...
    func deletePhase(phaseId: String) async throws {
        ...
        do {
            ...
            phases.removeAll { $0.id.uuidString == phaseId }
