                        ...
                        NoteCard(note: note)
                            .swipeActions(edge: .trailing, allowsFullSwipe: true) {
                                Button(role: .destructive) {
                                    Task {
                                        await viewModel.deleteNote(note.id.uuidString, for: patientId)
                                    }
                                } label: {
...
class NotesViewModel: ObservableObject {
    ...
    func saveNote(_ note: CreateNoteInput) async {
        ...
        logger.log("📝 Note type: \(note.noteType), session: \(note.sessionId?.uuidString ?? "nil")")
...
struct AddNoteSheet: View {
    ...
    private func saveNote() {
        ...
        // Convert String IDs to UUID for CreateNoteInput
        ...
        let sessionUUID: UUID? = if let sid = sessionId {
            UUID(uuidString: sid)
        } else {
            nil
        }
